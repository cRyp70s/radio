...
def get_from_url(playlist, song, song_url):
    ...
    if not os.path.exists(filename) or \
        (time.time() - os.path.getmtime(filename)) > CACHE_FILE_TIME:
        ...
        for _ in range(5):
            ...
            if resp.ok:
                ...
                with open(filename, "wb") as f:
                    f.write(data)
                com = ['ffmpeg', "-y", "-i", filename, "-c:a", "libopus", "-b:a", "40K", filename2]
                sp.check_output(com)
                break
        else:
        ...
        try:
            while True:
                ...
                for song_name, url, thumb_url in songs:
                    ...
                    sl = float(sp.Popen(f'ffprobe -i {fp} -show_entries format=duration -v quiet -of csv="p=0"', shell=True, stdout=sp.PIPE).stdout.read())
                    sleep = sl*0.95
                    logging.debug(f"Waiting for {song_name} to finish play {sleep}.")
                    playlist, songs = redis_backend.get("CURRENT_PLAY")
